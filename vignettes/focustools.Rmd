...
First, let's retrieve COVID-19 data from the JHU source. Note that the `get_cases()` and `get_deaths()` functions are executed separately, then joined and converted to a [`tsibble`](https://otexts.com/fpp3/tsibbles.html). Note that `granularity="state"` would pull in data at the state and territory level.
