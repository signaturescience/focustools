...
First, load ancillary packages for data analysis and retrieve COVID-19 data from the NYT source. Note that the `get_cases()` and `get_deaths()` functions are executed separately, then joined and converted to a `tsibble`:
