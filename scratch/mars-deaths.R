## initial exploration with MARS model and lagged predictor (deaths)
...
usa <-
  get_deaths(source = "jhu") %>%
  rename(week = epiweek) %>%
  filter(week != lubridate::week(Sys.Date())) %>%
  mutate(lag_deaths = lag(ideaths, default = 0L))
...
predict(usa_mars_fit, newdata = usa_test, interval = "pint") %>%
  ...
  mutate(epiyear = usa_test$epiyear) %>%
  ...
  ## line for observed
  geom_line(aes(date, ideaths), lwd = 1.25) +
  ## line for predicted
  geom_line(aes(date, fit), col = "firebrick", lwd = 1.25) +
  theme_minimal() +
  scale_y_continuous(labels=scales::number_format()) +
  labs(x = "Date", y = "Weekly COVID-19 Deaths (National)")
...
for(i in 1:horizon) {
  ...
  if(i == 1) oneweek_horizon <- tail(usa_train$ideaths, 1)
  ...
  usa_future <- tibble(week = whead, lag_deaths = oneweek_horizon)
